...
BATCH_SIZE = 64  # encode ทีละ 64 chunks เพื่อไม่ให้ RAM พัง


@lru_cache(maxsize=1)
def _get_model() -> SentenceTransformer:
    # lru_cache → โหลดครั้งเดียว ใช้ซ้ำตลอด process
...
def _get_collection() -> chromadb.Collection:
    # lru_cache → เปิด connection ครั้งเดียว
...
def embed_and_store(chunks: list[dict]) -> int:
    ...
    # encode เป็น batch เพื่อใช้ vectorized ops ของ numpy ได้เต็มที่
    ...
    # upsert = insert + update ในคำสั่งเดียว (ไม่ต้องเช็คซ้ำเอง)
...
def query_similar(
    ...
    return [
        {"text": doc, "source": meta.get("source", ""), "chunk_index": meta.get("chunk_index", -1)}
