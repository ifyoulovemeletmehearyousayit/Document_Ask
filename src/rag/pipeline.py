...
AVG_CHARS_PER_TOKEN = 4       # ประมาณการณ์หยาบๆ (ภาษาไทย ~3, อังกฤษ ~4)
SUMMARY_CHUNK_LIMIT = 30      # cap ไม่ให้ context ใหญ่เกินไปตอน summarize
...
def _build_prompt(question: str, context: str, is_thai: bool, is_summary: bool) -> str:
    lang_inst = (
        "IMPORTANT: You MUST answer in Thai (ภาษาไทย)."
        if is_thai else
        "Answer in English."
    )
    task_hint = (
...
def ask_question(question: str, source_filter: Optional[str] = None) -> str:
    ...
    if is_summary:
        # summary → ดึงทุก chunk แล้ว sort ตามลำดับ
        ...
        # QA → ดึงเฉพาะ chunk ที่ semantic คล้ายคำถาม
        chunks = query_similar(question, n_results=5, source_filter=source_filter)

    if not chunks:
        return (
            "ขออภัยครับ ไม่พบข้อมูลที่เกี่ยวข้องในเอกสารที่เลือก"
            if is_thai else
    ...
    try:
        ...
        return (
            "เกิดข้อผิดพลาดในการเชื่อมต่อกับ LLM กรุณาลองใหม่"
            if is_thai else
            "Failed to connect to LLM. Please try again."
        )